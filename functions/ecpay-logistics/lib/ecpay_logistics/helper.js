...
class APIHelper {
    ...
    http_request(method, api_url, payload){
        ...
        return new Promise((resolve, reject) => {
            const req = http_op.request(options, (res) => {
                ...
                // functions.logger.log(`HEADERS: ${JSON.stringify(res.headers)}`);
            ...
            req.on('error', (e) => {
                functions.logger.error(`problem with request: ${e.message}`);
                reject(e)
            });
    ...
    gen_html_post_form(act, id, parameters, input_typ='hidden', submit=true){
        ...
        html += "</form>";
        // functions.logger.log(typeof html);
        return html
    }
