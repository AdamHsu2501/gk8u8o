...
class LogisticsVerifyBase {
    ...
    verify_param_by_pattern(params, pattern) {
        ...
        Object.keys(params).forEach(function (p_name) {
            ...
            // functions.logger.log('p_name', p_name)
            // functions.logger.log('p_type', p_type)
            ...
            switch (p_type) {
                case 'String':
                    ...
                    if (mat === null) {
                        functions.logger.error(`Wrong format of param ${p_name} or length exceeded.`);
                ...
                case 'Opt':
                    ...
                    if (mat_opt === false) {
                        functions.logger.error(`Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_opt}).`);
                    ...
                    switch (mode) {
                        case 'BETWEEN':
                            if (val < min || val > max) {
                                ...
                                functions.logger.error(`Value of ${p_name} should be between ${min} and ${max}.`);
                        ...
                        case 'GE':
                            if (val < min) {
                                functions.logger.error(`Value of ${p_name} should be greater than or equal to ${min}.`);
                        ...
                        case 'LE':
                            if (val > max) {
                                functions.logger.error(`Value of ${p_name} should be less than or equal to ${max}.`);
                        ...
                        case 'EXCLUDE':
                            if (val >= max && val <= max) {
                                functions.logger.error(`Value of ${p_name} can NOT be between ${min} and ${max}..`);

                                throw new ECpayError.ECpayInvalidParam(`Value of ${p_name} can NOT be between ${min} and ${max}..`);
                            }
                            break;
                        default:
                            functions.logger.error(`Unexpected integer verification mode for parameter ${p_name}: ${mode}.`);
                ...
                case 'DepOpt':
                    ...
                    if (aval_dopt === null && pattern['Opt'][parent_param].includes(parent_val) === false) {
                        functions.logger.error(`Cannot find available option of [${p_name}] by related param [${parent_param}](Value: ${parent_val}.`);
                        ...
                        if (!aval_dopt.includes(params[p_name])) {
                            functions.logger.error(`Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_dopt}).`);
                    ...
                    break;
                default:
                    functions.logger.error(`Unexpected type (${p_type}) for parameter ${p_name}.`);
...
class QueryParamVerify extends LogisticsVerifyBase {
    ...
    verify_query_param(params) {
        if (params.constructor === Object) {
            ...
            basic_param.forEach(function (pname) {
                if (input_param.indexOf(pname, 0) === -1) {
                    functions.logger.error(`Lack required param ${pname}`)
                    throw new ECpayError.ECpayInvalidParam(`Lack required param ${pname}`);
                }
            });
            ...
            functions.logger.error(`Received argument is not a object`)
